...
const TiptapEditor = dynamic(() => import('@/components/editor/TiptapEditor'), {
    ssr: false,
    loading: () => <div className="h-[calc(100vh-200px)] w-full bg-gray-50 animate-pulse rounded-lg" />
})
...
export default function NoteClientWrapper({ id, initialData, initialContent }: NoteClientWrapperProps) {
                ...
                >
                    ...
                    {editorMode === 'rich' ? (
                        <div ref={editorContainerRef} className="space-y-3" style={isFullscreen ? { position: 'fixed', inset: 0, zIndex: 50, width: '100vw', height: '100vh', background: 'transparent' } : undefined}>
                            {!isEditorMounted ? (
                                <div
                                    className="prose max-w-none p-4 min-h-[calc(100vh-200px)]"
                                    ...
                                    {showInsertMenu && (
                                        <div className="fixed left-6 bottom-40 z-50 rounded-xl border bg-white shadow-xl" role="menu" aria-label="插入工具菜单" style={{ minWidth: 220 }}>
                                            <div className="p-2 grid" style={{ rowGap: 6 }}>
                                                ...
                                                <button role="menuitem" className="text-left px-3 py-2 hover:bg-gray-50" onClick={async () => { setShowInsertMenu(false); try { const res = await boardsAPI.create('画板', id); document.dispatchEvent(new CustomEvent('tiptap:exec', { detail: { cmd: 'insertResource', payload: { type: 'board', id: res.id } } })) } catch { } }}>画板</button>
