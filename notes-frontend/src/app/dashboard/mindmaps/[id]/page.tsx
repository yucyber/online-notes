...
export default function MindmapDetailPage() {
  ...
  const [map, setMap] = useState<{ id: string; title: string } | null>(null)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold">思维导图：{map.title}</h1>
        <Button variant="ghost" onClick={() => router.push('/dashboard/notes')}>返回笔记</Button>
      </div>
      <div className="rounded-lg border bg-white p-4">
        <div className="text-sm text-gray-600">资源ID：{map.id}</div>
        <div className="mt-3 text-sm text-gray-600">功能占位：后续可接入Mindmap编辑器。</div>
      </div>
    </div>
  )
}
