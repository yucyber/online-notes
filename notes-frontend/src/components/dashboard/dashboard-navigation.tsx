...
// 匹配 /dashboard/notes/<id> 以及 /dashboard/notes/<id>/<sub>，把 <id> 段渲染为笔记标题
const NOTE_SEGMENT_PATTERN = /^\/dashboard\/notes\/([^/]+)(?:\/([^/]+))?$/
...
export function DashboardHeader({ pathname, user, isDark, isSidebarHidden, unreadCount, onToggleSidebar, onToggleTheme, onNavigate, onLogout }: { pathname: string | null; user: User | null; isDark: boolean; isSidebarHidden: boolean; unreadCount: number; onToggleSidebar: () => void; onToggleTheme: () => void; onNavigate: (href: string) => void; onLogout: () => void }) {
  ...
  const noteTitleMap = useNoteTitleMap(pathname)
  return <header className="dashboard-topbar"><div className="dashboard-topbar-left"><button className="prototype-icon-button" onClick={onToggleSidebar} aria-label={isSidebarHidden ? '显示侧边栏' : '隐藏侧边栏'}><svg><path d="M4 6h16M4 12h16M4 18h16"/></svg></button><Breadcrumb pathname={pathname} onNavigate={onNavigate} extraLabels={noteTitleMap} /></div><div className="dashboard-topbar-right"><button className="prototype-icon-button" aria-label={isDark ? '切换到浅色主题' : '切换到深色主题'} aria-pressed={isDark} onClick={onToggleTheme}><svg><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z"/></svg></button><button className="prototype-icon-button dashboard-notification" onClick={() => onNavigate('/dashboard/notifications')} aria-label={unreadCount > 0 ? `消息中心，未读 ${unreadCount} 条` : '打开消息中心'}><svg><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></button><div className="relative"><button className="dashboard-avatar" aria-label="打开用户菜单" aria-expanded={isUserMenuOpen} onClick={() => setIsUserMenuOpen((open) => !open)}>{displayName.charAt(0).toUpperCase()}</button>{isUserMenuOpen && <div role="menu" tabIndex={-1} className="dashboard-user-menu"><div>{displayName}</div><button role="menuitem" onClick={() => { setIsUserMenuOpen(false); onNavigate('/dashboard/settings') }}>设置</button><button role="menuitem" onClick={onLogout}>退出登录</button></div>}</div></div></header>
}

function Breadcrumb({ pathname, onNavigate, extraLabels }: { pathname: string | null; onNavigate: (href: string) => void; extraLabels?: Record<string, string> }) {
  const segments = pathname?.split('/').filter(Boolean) || []
  return <nav aria-label="面包屑" className="dashboard-crumbs">{segments.map((segment, index) => { const isLast = index === segments.length - 1; const path = `/${segments.slice(0, index + 1).join('/')}`; const label = routeNames[segment] || extraLabels?.[segment] || (segment.length > 20 ? `${segment.slice(0, 8)}...` : segment); return <span key={path}>{index > 0 && <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>}{isLast ? <b>{label}</b> : <button onClick={() => onNavigate(path)}>{label}</button>}</span> })}</nav>
}

// 从 pathname 中提取笔记 id，按需拉取标题；只在 id 段未命中缓存时发起请求，
// 避免在编辑工作区或非笔记子页面产生额外流量。
function useNoteTitleMap(pathname: string | null): Record<string, string> {
  const [titles, setTitles] = useState<Record<string, string>>({})
  useEffect(() => {
    if (!pathname) return
    const match = pathname.match(NOTE_SEGMENT_PATTERN)
    const noteId = match?.[1]
    if (!noteId) return
    if (noteId === 'new' || titles[noteId]) return
    let cancelled = false
    fetchNoteById(noteId)
      .then((note: any) => {
        if (cancelled) return
        const title = typeof note?.title === 'string' && note.title.trim() ? note.title.trim() : '无标题笔记'
        setTitles((prev) => (prev[noteId] === title ? prev : { ...prev, [noteId]: title }))
      })
      .catch(() => { /* 保留 fallback 显示原 id，截断由 Breadcrumb 处理 */ })
