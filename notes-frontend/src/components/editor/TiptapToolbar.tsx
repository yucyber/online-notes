...
export default function TiptapToolbar({ disabled, exec, isFullscreen }: Props) {
  ...
  return (
    <div
      ...
      style={{ height: 44, paddingLeft: 0, paddingRight: 16, border: '1px solid var(--border)', borderRadius: 12, background: 'var(--surface-1)', overflowX: 'auto', overscrollBehaviorX: 'contain', gap: 2 }}
    >
      {/* 基本文本样式 - 图标化 */}
      ...
      <select
        ...
        onChange={(e) => {
          ...
          if (v === 'paragraph') exec('paragraph')
          else if (v.startsWith('h')) exec('heading', { level: Number(v.substring(1)) })
        }}
        defaultValue="paragraph"
        style={{ height: 32, border: '1px solid var(--border)', borderRadius: 8, padding: '0 6px', marginRight: 2 }}
      >
      ...
      <select
        ...
        title="字号"
        disabled={disabled}
        onChange={(e) => exec('fontSize', { size: `${e.target.value}px` })}
        defaultValue="15"
        style={{ height: 32, border: '1px solid var(--border)', borderRadius: 8, padding: '0 6px', marginRight: 2 }}
      >
      ...
      {/* 链接/图片/表格 */}
      <Button size="icon" variant="ghost" aria-label="插入链接" title="插入链接 (Ctrl+K)" disabled={disabled} onClick={() => { try { document.dispatchEvent(new CustomEvent('open:link-dialog')) } catch { } }}><LinkIcon className="w-4 h-4" aria-hidden /></Button>
      ...
      <input id={fileInputId} type="file" accept="image/*" className="sr-only" disabled={disabled} onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const reader = new FileReader(); reader.onload = () => { const src = String(reader.result || ''); exec('image', { src }) }; reader.readAsDataURL(f); e.currentTarget.value = '' }} />
      ...
      <div className="flex items-center gap-2">
        ...
        <input type="color" aria-label="文字颜色" title="文字颜色" disabled={disabled} onChange={(e) => exec('color', { color: e.target.value })} style={{ width: 28, height: 28, border: '1px solid var(--border)', borderRadius: 6, background: 'var(--surface-1)' }} />
        {/* 撤销/重做/保存 */}
        ...
        {/* 评论按钮（打开评论弹窗） */}
        <Button size="icon" variant="ghost" aria-label="打开评论" title="打开评论 (Alt+C)" disabled={disabled} onClick={() => exec('comments')}>
