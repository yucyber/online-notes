...
function CategoryRow({ category, selected, parentName, onToggle, onEdit, onDelete }: { category: Category; selected: boolean; parentName?: string; onToggle: () => void; onEdit: () => void; onDelete: () => void }) {
  return <div className="flex flex-col gap-4 rounded-xl border border-[var(--product-line)] bg-[var(--product-panel)] p-4 transition-colors hover:border-[var(--product-accent-line)] md:flex-row md:items-center md:justify-between"><div className="flex flex-1 items-start gap-4"><input type="checkbox" checked={selected} onChange={onToggle} className="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" /><div className="mt-1 h-4 w-4 rounded-full ring-2 ring-[var(--product-line-soft)]" style={{ backgroundColor: category.color || DEFAULT_CATEGORY_COLOR }} /><div className="flex-1"><div className="mb-1 flex items-center gap-3"><p className="font-semibold text-[var(--product-text)]">{category.name}</p><span className="rounded-full border border-[var(--product-line-soft)] bg-[var(--product-panel-soft)] px-2.5 py-1 text-xs text-[var(--product-text-secondary)]">{category.noteCount ?? 0} 条笔记</span></div>{category.description && <p className="mt-2 text-sm leading-relaxed text-[var(--product-text-secondary)]">{category.description}</p>}{parentName && <p className="mt-2 text-xs text-[var(--product-accent)]">隶属于：{parentName}</p>}<p className="mt-3 text-xs text-[var(--product-muted)]">更新于 {formatDate(category.updatedAt)}</p></div></div><div className="flex items-center gap-2"><Button variant="ghost" size="sm" onClick={onEdit}><Pencil className="mr-1 h-4 w-4" />{"编辑"}</Button><Button variant="ghost" size="sm" className="text-[var(--product-danger)] hover:bg-[var(--product-danger-soft)]" onClick={onDelete}><Trash2 className="mr-1 h-4 w-4" />{"删除"}</Button></div></div>
}
