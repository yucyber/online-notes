...
describe('编辑器真实 CSS 响应式契约', () => {
  test('1023px 以下切为单列覆盖层并限制正文和工具栏横向溢出', () => {
    ...
    expect(declarations(findRule(compact, '.editor-left-navigation', 'position')).get('position')?.value).toBe('absolute')
    expect(declarations(findRule(compact, '.editor-right-metadata', 'position')).get('position')?.value).toBe('absolute')
    expect(declarations(findRule(compact, '.editor-left-navigation', 'left')).get('left')?.value).toBe('24px')
    expect(declarations(findRule(compact, '.editor-left-navigation', 'width')).get('width')?.value).toBe('var(--editor-left-width)')
    expect(declarations(findRule(compact, '.editor-right-metadata', 'right')).get('right')?.value).toBe('24px')
    expect(declarations(findRule(compact, '.editor-layout-grid[data-right-collapsed="true"] .editor-right-metadata')).get('width')?.value)
      .toBe('52px')
