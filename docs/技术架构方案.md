...
- **框架**: Next.js 16 (App Router) - 利用服务端组件 (RSC) 提升首屏性能与 SEO。
...
#### 3.2.3 AI 智能摘要与列表性能优化
针对笔记列表页加载慢、流量大的痛点，实施了以下架构升级：
1. **数据库变更**: `Note` 模型新增 `summary` 字段。
2. **异步生成**: 
   - 笔记保存时，后端先同步截取前 200 字作为兜底摘要。
   - 随后异步触发 Coze AI 任务，生成 200 字左右的精简摘要并覆盖存储。
3. **接口瘦身**: 列表接口 (`GET /notes`) 彻底移除 `content` 字段，仅返回 `summary`，数据传输量降低 90% 以上。
4. **前端交互**: 引入“悬浮预览”机制，鼠标停留显示完整摘要，支持滚动查看，兼顾列表整洁度与信息获取效率。
...
- **Cookie 同步**: 前端登录成功后，将 Token 同步写入 Cookie (`notes_token`)，有效期 1 天，以便服务端组件 (RSC) 进行鉴权。
...
- **渲染优化**: 笔记详情页采用 **即时预览 (Instant Preview)** 策略。
  - 使用服务端组件 (RSC) 直接获取数据，消除客户端请求瀑布流。
  - 首屏直接返回静态 HTML 内容，编辑器加载完成后无缝接管 (Hydration)，显著降低 LCP (Largest Contentful Paint)。
