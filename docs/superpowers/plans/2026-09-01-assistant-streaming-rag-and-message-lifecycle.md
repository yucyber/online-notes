...
- Produces:
  ...
  - `export type AssistantMessageView = { id: string; conversationId: string; seq: number; role: 'user' | 'assistant'; route: AssistantRoute; content: string; status: 'pending' | 'streaming' | 'completed' | 'failed' | 'cancelled'; requestId?: string; retryOfMessageId?: string; citations: RagCitation[]; warnings: string[]; tokenUsage?: { input: number; output: number }; createdAt: string; completedAt?: string }`
