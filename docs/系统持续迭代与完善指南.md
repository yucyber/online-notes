...
- **前端**: 
  - 推送至 GitHub `main` 分支。
  - Vercel 自动触发构建与部署。
  - Vercel 只配置前端公开变量，例如 `NEXT_PUBLIC_API_URL`。模型密钥统一放在后端运行环境，不再配置前端 Coze key。
  - 后端运行环境需配置 `MIMO_*`、`SENSENOVA_*`、`SILICONFLOW_*` 和 `AI_*_PROVIDER`，提交前运行 `npm run check:ai-config`。
- **后端/协作**: 
