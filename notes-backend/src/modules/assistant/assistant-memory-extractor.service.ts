  ...
  async extract(userId: string, conversationId: string): Promise<{ created: number; skipped: number }> {
    ...
    try {
      const result = await this.gateway.chatTask({
        ...
        maxTokens: 512,
        temperature: 0,
        system: `Extract durable memory candidates from this conversation. Each transcript line starts with its message id as [m:<message-id>]. Allowed kinds: ${MEMORY_KINDS.join(', ')}. Return JSON only: {"candidates":[{"kind":"...","subject":"short topic","statement":"one-sentence fact/decision","confidence":0-1,"messageIds":["..."]}]}, where "messageIds" must contain exactly the raw <message-id> values (24-hex ids, WITHOUT the "m:" prefix) of the lines that support the candidate. Write "subject" and "statement" in the SAME LANGUAGE as the conversation (Chinese conversation → Chinese output; keep unavoidable technical terms like "React/Vue/NestJS" as-is); this is required so later recall by Chinese questions can match them. Rules: ignore small talk, emotions, and speculation. A suggestion made only by the assistant and not confirmed by the user must use kind "hypothesis". Do not invent facts.`,
