...
test('内容哈希未变化的 Chunk 复用已有 embedding', async () => {
  ...
  const service = new NoteChunkIndexService(
    ...
    { exists: async () => ({ _id: '66c1c0e0a1b2c3d4e5f6a7b8' }) } as any,
  ...
  const result = await service.refreshNoteChunks({
    noteId: '66c1c0e0a1b2c3d4e5f6a7b8', userId: '66c1c0e0a1b2c3d4e5f6a7c9', title: 'React', content: 'Diff', expectedUpdatedAt: updatedAt,
  })
...
test('只为变化的 Chunk 生成 embedding 并替换失效位置', async () => {
  ...
  const result = await service.refreshNoteChunks({
    noteId: '66c1c0e0a1b2c3d4e5f6a7b8', userId: '66c1c0e0a1b2c3d4e5f6a7c9', title: 'React', content: '正文', expectedUpdatedAt: updatedAt,
  })
...
test('写入 Chunk 时 noteId/userId 存储为 ObjectId，与读取方的 ObjectId 查询一致', async () => {
  const bulkOperations: any[] = []
  const service = new NoteChunkIndexService(
    {
      find: () => queryResult([]),
  ...
  await service.refreshNoteChunks({
    noteId: '66c1c0e0a1b2c3d4e5f6a7b8', userId: '66c1c0e0a1b2c3d4e5f6a7c9',
    title: 'Title', content: 'body', expectedUpdatedAt: updatedAt,
  })

  const replacement: any = bulkOperations[0].replaceOne.replacement
  assert.equal(replacement.userId instanceof Types.ObjectId, true)
  assert.equal(replacement.noteId instanceof Types.ObjectId, true)
  assert.equal(String(replacement.userId), '66c1c0e0a1b2c3d4e5f6a7c9')
  assert.equal(String(replacement.noteId), '66c1c0e0a1b2c3d4e5f6a7b8')
})
